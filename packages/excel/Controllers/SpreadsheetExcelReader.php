 ...
 * @link	   http://pear.php.net/package/SpreadsheetExcelReader
...
function gmgetdate($ts = null) {
    ...
    /**
      function split is removed in php 7...Rituraj 10/10/2017
     * */
...
class SpreadsheetExcelReader {
    ...
    function hyperlink($row, $col, $sheet = 0) {
        $link = !empty($this->sheets[$sheet]['cellsInfo'][$row][$col]['hyperlink']) 
                ? $this->sheets[$sheet]['cellsInfo'][$row][$col]['hyperlink'] : '' ;
        if ($link) {
            return $link['link'];
        }
        return '';
    ...
    /* Modified the above function to get data in excel sheet in array
      Instead of printing it in tabular form...Rituraj 10/10/2017
     */
    function getDataArray($row_numbers = false, $col_letters = false, $sheet = 0, $table_class = 'excel') {
        $out = [];
        ...
        for ($row = 1; $row <= $rowCount; $row++) {
            ...
            for ($col = 1; $col <= $colCount; $col++) {
                ...
                $val = trim($this->val($row, $col, $sheet));
    ...
    function __construct($store_extended_info = true, $outputEncoding = '') {
        ...
        /*
          Corrected this line as the above line was giving parse error below...Rituraj 10/10/2017 :
          Parse error: syntax error, unexpected 'new' (T_NEW) in D:\xampp\htdocs\test\excel\excel_reader2.php on line 916
         */
        $this->_ole = new OLERead();
