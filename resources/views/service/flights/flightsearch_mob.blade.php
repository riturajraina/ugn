...
<div class="card pl-3 pr-3 pb-3 mb-3 service-form">
  ...
  <div class="mt-2">
	 ...
	 <div class="input-group mb-3 mb-sm-0 form-group">
            ...
            value="<?php echo !empty($dateFrom) ? $dateFrom : '';?>" onmouseover="javascript:emptyValue();">
            <input type="hidden" id="departureDate" name="departureDate"
                   value="<?php echo !empty($departureDate) ? $departureDate : '';?>">
            <input type="hidden" id="dateToPrevious">
	 </div>
	 ...
	 <div class="input-group mb-3 mb-sm-0 form-group" id="returnDayDiv"
         <?php echo $way == 1 ? 'style="display:none;"' : '';?>>
            ...
            <?php echo $way == 1 ? 'disabled' : '';?> onmouseover="javascript:emptyValueReturn();">
            <input type="hidden" id="arrivalDate" name="arrivalDate" 
                    value="<?php echo !empty($arrivalDate) ? $arrivalDate : '';?>">
            <input type="hidden" id="dateFromPrevious">
...
<?php
    $datesJs = "";
    //echo '<br>$futurePriceArray from view : <pre>' . print_r($futurePriceArray, true) . '</pre>';exit;
    if (!empty($futurePriceArray) && is_array($futurePriceArray)) {
        if (count($futurePriceArray)) {
            foreach ($futurePriceArray as $futureDate=>$futurePriceDateArray) {
                $datesJs    .=  'dates[\'' . $futureDate . '\']=\'र ' . $futurePriceDateArray['minPriceDeparture'] . '\';' . "\n";
            }
        }
    }
...
function emptyValue()
{
  $('#dateToPrevious').val($('#dateFrom').val());
  $('#dateFrom').val('');
  //$('#departureDate').val('');
}

function emptyValueReturn()
{
  $('#dateFromPrevious').val($('#dateTo').val());
  $('#dateTo').val('');
  //$('#arrivalDate').val('');
}

$( "#dateFrom" ).mouseout(function() {
  var predateFrom = $('#dateToPrevious').val();
  $( "#dateFrom" ).val(predateFrom);
});

$( "#dateTo" ).mouseout(function() {
  var predateTo = $('#dateFromPrevious').val();
  $( "#dateTo" ).val(predateTo);
});
</script>
