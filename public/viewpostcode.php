            ...
            <div class="ugn-upper-content">
               <div class="row p-3">
					<div class="col-3 pr-2">
						<?php
							if (!empty($content['header']['header_images']) && is_array($content['header']['header_images']) && count($content['header']['header_images'])) {
								foreach ($content['header']['header_images'] as $imageSource) {
									echo '<img class="img-fluid" src="\images\ugn\\' . $imageSource . '">';
								}
							} else {
						?>		<img class="img-fluid" src="<?php echo $content['header']['header_img_1'];?>">
						<?php
							}
						?>
					...
					foreach ($tabArray as $accordion) {
						...
						for ($k=1;$k < $accordionCount; $k++) {
							...
							if (!empty($accordion['acc_' . $k]['acc_title_text'])) {
								...
								$htmlString .= '<p>' . $accordion['acc_' . $k]['acc_para_text'] . '</p>';
							}
						}
